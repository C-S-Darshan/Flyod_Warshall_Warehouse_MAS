...
# --- Core Simulation Logic (Unidirectional) ---
...
def build_graph(rows, cols, shelves_set):
    """
    Builds the adjacency matrix based on strict unidirectional lanes.
    """
    N = rows * cols
...
def dijkstra_with_avoidance(start_id, goal_id, start_time, occupied_at_time, max_time, rows, cols, shelves_set):
    """
    Rerouting with Dijkstra still considers all 4 directions to find a way around a conflict.
    """
    ...
    while pq:
        ...
        for dr, dc in [(0, 1), (0, -1), (1, 0), (-1, 0)]: # All 4 directions for rerouting
            nr, nc = r + dr, c + dc
            if in_bounds(nr, nc, rows, cols) and (nr, nc) not in shelves_set:
                neighbor_id = node_id(nr, nc, cols)
                next_time = cost + 1
...
def run_simulation_logic(rows: int, cols: int, shelves: List[Tuple[int, int]], agents: List[Agent]):
    ...
    for agent_config in agents:
        ...
        if not initial_path_ids:
            logs.append(f"❌ Agent {agent_id}: No initial path found in the unidirectional grid.")
            final_agent_paths.append({"id": agent_id, "path": [], "rerouted": False, "initial_path": []})
            continue
